...
ikea |> filter(category == "Nursery furniture")
...
# If they don't filter, they cannot have the right answer.
# Their plot has to look like this.
ikea |>
  filter(category == "Sofas & armchairs") |>
  ggplot(aes(x = width, y = price)) +
  geom_point()
...
- **Answer:** 19.88. For every one unit increase in width, they price goes up by
  nearly 20 Riyals.
- Helpful Hints:
...
sofas <- ikea |> filter(category == "Sofas & armchairs")

linear_reg() |> fit(price~width, data = sofas)
...
- **Answer:** Avg: Basically 0. SD: 1157

```{r}

lm_width <- linear_reg() |> fit(price~width, data = sofas)
...
sofas |>
  summarize(
    avg_resid = mean(.resid, na.rm = TRUE),
